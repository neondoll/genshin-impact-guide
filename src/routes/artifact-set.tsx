...
export default function ArtifactSet() {
  const { artifactSetKey } = useParams();
  const [artifactSet, setArtifactSet] = useState<ReturnType<typeof selectArtifactSetById>>();
  const [artifactSetRecommendations, setArtifactSetRecommendations] = useState<ReturnType<typeof selectArtifactSetRecommendationsById>>();

  useEffect(() => {
    const artifactSet = selectArtifactSetById(artifactSetKey as TArtifactSetKey);

    setArtifactSet(artifactSet);
    setArtifactSetRecommendations(selectArtifactSetRecommendationsById(artifactSet.key));
  }, [artifactSetKey]);
