...
export default function WeaponsTierListPage() {
  const { tierListsWeapons } = useLoaderData<WeaponsTierListLoaderReturn>();
      ...
      <Tabs defaultValue={tierListsWeapons[0]?.title}>
        ...
        {tierListsWeapons.map((tierListWeapons) => (
          <TabsContent className="flex flex-col gap-2" key={tierListWeapons.title} value={tierListWeapons.title}>
            ...
            {tierListWeapons.video_source_id && (
              <Card>
                <CardHeader>
                  <CardTitle children="Видео-источник" />
                </CardHeader>
                <CardContent>
                  <VideoSources items={selectVideoSourcesByIds([tierListWeapons.video_source_id])} />
                </CardContent>
              </Card>
            )}
          </TabsContent>
