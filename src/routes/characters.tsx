...
import {
  selectElementById,
  selectElementsAll,
} from "@/features/elements/selectors";
...
export function loader() {
  ...
  const rarities = selectRaritiesByIds(
    characters
  ...
  useEffect(() => {
    ...
    if (filterElementIds.length) {
      filteredCharacters = filteredCharacters.filter((character) =>
        filterElementIds.includes(character.element_id)
      );
    }
    ...
    if (filterRarities.length) {
      filteredCharacters = filteredCharacters.filter(
        (character) =>
          character.rarity && filterRarities.includes(character.rarity)
      );
    }
    ...
    if (filterWeaponTypeIds.length) {
      filteredCharacters = filteredCharacters.filter(
        (character) =>
          ...
          && filterWeaponTypeIds.includes(character.weapon_type_id)
      );
    }
                ...
                onChange={(event) => {
                  if (event.target.checked) {
                    if (!filterElementIds.includes(element.id)) {
                      setFilterElementIds((prev) =>
                        prev.concat([element.id])
                      );
                    }
                  }
                  else {
                ...
                onChange={(event) => {
                  if (event.target.checked) {
                    if (!filterWeaponTypeIds.includes(weaponType.id)) {
                      setFilterWeaponTypeIds((prev) =>
                        prev.concat([weaponType.id])
                      );
                    }
                  }
                  else {
      ...
      <ul className="flex flex-wrap gap-2 justify-center items-stretch md:gap-4">
        {filteredCharacters.map((character) => {
          ...
          return (
            <li
              className={cn(
                "flex relative flex-col gap-4 px-5.5 py-4 w-36 h-45",
                "text-card-foreground bg-card rounded-xl border shadow-sm",
                "transition-all has-hover:scale-104 has-focus-visible:ring-3",
            ...
            >
              ...
              <Link
                ...
                className={cn(
                  "inline-flex flex-1 justify-center items-center text-sm",
                  "text-center outline-none before:absolute before:inset-0"
                )}
