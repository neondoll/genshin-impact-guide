...
import type { ResourceFood, ResourceId } from "@/features/resources/types";
...
import { selectResourceById } from "@/features/resources/selectors";
...
export function loader({ params }: { params: Record<string, string | undefined> }) {
  ...
  const recipeDishes: ResourceFood[] = [];

  return { recipeDishes, resource, resourceType };
}

export default function Resource() {
  const { recipeDishes, resource, resourceType } = useLoaderData<ReturnType<typeof loader>>();
                  ...
                  <TableCell className="p-2 text-pretty whitespace-normal">
                    <ul>
                      {recipeDishes.map(recipeDish => (
                        <li key={recipeDish.id}>{recipeDish.id}</li>
                      ))}
                    </ul>
                  </TableCell>
                </TableRow>
