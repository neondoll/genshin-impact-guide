...
function HomeList({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      className={cn(
...
function HomeListItem({ className, imageSrc, label, ...props }: {
  ...
  return (
    <li
      ...
      {...props}
    >
      <span className="aspect-square inline-flex shrink-0 justify-center items-center w-full h-auto bg-linear-to-br from-muted to-muted-foreground rounded-t-xl">
        {imageSrc !== undefined && (
...
export async function loader() {
  ...
  return { characters, elements, regions, weapons, weaponTypes } as HomeLoaderData;
}

export default function Home() {
  const { characters, elements, regions, weapons, weaponTypes } = useLoaderData<HomeLoaderData>();
