...
export default new ArtifactSetRecommendations(ArtifactSetIds.ViridescentVenerer)
  .setCharacters([
    new ArtifactSetCharacterRecommendation(CharacterIds.Jean).setIsBetter().setNotes([
      ...
      + artifactSetById(ArtifactSetIds.MaidenBeloved) + " или аналогами, если команде не хватает лечения, или с 2 "
      + "предметами " + artifactSetById(ArtifactSetIds.GladiatorsFinale) + " (или аналогами), так как лечение Джинн "
      + "зависит от силы атаки. 2 предмета набора " + artifactSetById(ArtifactSetIds.ViridescentVenerer) + " в таком "
      + "случае можно заменить " + artifactSetById(ArtifactSetIds.DesertPavilionChronicle) + ".",
    ]),
